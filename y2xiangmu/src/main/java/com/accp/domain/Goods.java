package com.accp.domain;

public class Goods {
    private String goodsno;

    private String goodsname;

    private String goodsbank;

    private String shiyong;

    private String danwei;

    private Integer goodstypeid;

    private Integer shouruid;

    private String yuanfuchang;

    private String goodsgraden;

    private String goodsaddress;

    private String huoshangno;

    private String baozhuan;

    private Double tiji;

    private Double maozhong;

    private Double jingzhong;

    private String jinhuo;

    private Double shoujia;

    private String jiajia;

    private Double price;

    private Double vipprice;

    private Double huiprice;

    private String istiyong;

    public String getGoodsno() {
        return goodsno;
    }

    public void setGoodsno(String goodsno) {
        this.goodsno = goodsno;
    }

    public String getGoodsname() {
        return goodsname;
    }

    public void setGoodsname(String goodsname) {
        this.goodsname = goodsname;
    }

    public String getGoodsbank() {
        return goodsbank;
    }

    public void setGoodsbank(String goodsbank) {
        this.goodsbank = goodsbank;
    }

    public String getShiyong() {
        return shiyong;
    }

    public void setShiyong(String shiyong) {
        this.shiyong = shiyong;
    }

    public String getDanwei() {
        return danwei;
    }

    public void setDanwei(String danwei) {
        this.danwei = danwei;
    }

    public Integer getGoodstypeid() {
        return goodstypeid;
    }

    public void setGoodstypeid(Integer goodstypeid) {
        this.goodstypeid = goodstypeid;
    }

    public Integer getShouruid() {
        return shouruid;
    }

    public void setShouruid(Integer shouruid) {
        this.shouruid = shouruid;
    }

    public String getYuanfuchang() {
        return yuanfuchang;
    }

    public void setYuanfuchang(String yuanfuchang) {
        this.yuanfuchang = yuanfuchang;
    }

    public String getGoodsgraden() {
        return goodsgraden;
    }

    public void setGoodsgraden(String goodsgraden) {
        this.goodsgraden = goodsgraden;
    }

    public String getGoodsaddress() {
        return goodsaddress;
    }

    public void setGoodsaddress(String goodsaddress) {
        this.goodsaddress = goodsaddress;
    }

    public String getHuoshangno() {
        return huoshangno;
    }

    public void setHuoshangno(String huoshangno) {
        this.huoshangno = huoshangno;
    }

    public String getBaozhuan() {
        return baozhuan;
    }

    public void setBaozhuan(String baozhuan) {
        this.baozhuan = baozhuan;
    }

    public Double getTiji() {
        return tiji;
    }

    public void setTiji(Double tiji) {
        this.tiji = tiji;
    }

    public Double getMaozhong() {
        return maozhong;
    }

    public void setMaozhong(Double maozhong) {
        this.maozhong = maozhong;
    }

    public Double getJingzhong() {
        return jingzhong;
    }

    public void setJingzhong(Double jingzhong) {
        this.jingzhong = jingzhong;
    }

    public String getJinhuo() {
        return jinhuo;
    }

    public void setJinhuo(String jinhuo) {
        this.jinhuo = jinhuo;
    }

    public Double getShoujia() {
        return shoujia;
    }

    public void setShoujia(Double shoujia) {
        this.shoujia = shoujia;
    }

    public String getJiajia() {
        return jiajia;
    }

    public void setJiajia(String jiajia) {
        this.jiajia = jiajia;
    }

    public Double getPrice() {
        return price;
    }

    public void setPrice(Double price) {
        this.price = price;
    }

    public Double getVipprice() {
        return vipprice;
    }

    public void setVipprice(Double vipprice) {
        this.vipprice = vipprice;
    }

    public Double getHuiprice() {
        return huiprice;
    }

    public void setHuiprice(Double huiprice) {
        this.huiprice = huiprice;
    }

    public String getIstiyong() {
        return istiyong;
    }

    public void setIstiyong(String istiyong) {
        this.istiyong = istiyong;
    }
}